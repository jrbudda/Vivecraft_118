...
def createIdeaProject(project_root_dir, version, mcpDirName, are32bitNatives):
    ...
    # find objc library
    objcDir = os.path.join(project_root_dir, mcpDirName, 'jars', 'libraries', 'ca', 'weblite')
    for root, dirs, files in os.walk(objcDir):
        for file in files:
            if file.endswith("sources.jar"):
                sourceFile = os.path.join(root, file)
                sourcePaths.append(sourceFile)
    ...
    miscXML = MiscXML(languageLevel='JDK_17', projectjdkname="17")
...
""")
    ...
    def __init__(self, languageLevel, projectjdkname):
        ...
        component.attrib['project-jdk-name'] = projectjdkname
