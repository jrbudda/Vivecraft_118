...
def main(mcp_dir, version = "VR", orig_dir = ".minecraft_orig"):
    new_src_dir    = os.path.join( base_dir, "src" , version)
    patch_base_dir = os.path.join( base_dir, "patches", version)
    resources_base_dir    = os.path.join(base_dir, "resources", version)
...
if __name__ == '__main__':
    ...
    if not options.mcp_dir is None:
        main(os.path.abspath(options.mcp_dir), options.version, options.orig_dir)
    elif os.path.isfile(os.path.join('..', 'runtime', 'commands.py')):
        main(os.path.abspath('..'), options.version, options.orig_dir)
    else:
        main(os.path.abspath(mcp_version), options.version, options.orig_dir)
