...
public class KeyboardHandler
{
    ...
    public static boolean setOverlayShowing(boolean showingState)
    {
        ...
        else
        {
            ...
            if (showingState)
            {
                ...
                if (mc.vrSettings.physicalKeyboard && mc.screen != null)
                {
        ...
        if (Showing)
        {
            if (!mc.vrSettings.seated)
            {
                if (Rotation_room != null)
                {
                    ...
                    {
                        Vec2 vec2 = GuiHandler.getTexCoordsForCursor(Pos_room, Rotation_room, mc.screen, GuiHandler.guiScale, mc.vrPlayer.vrdata_room_pre.getController(1));
                        Vec2 vec21 = GuiHandler.getTexCoordsForCursor(Pos_room, Rotation_room, mc.screen, GuiHandler.guiScale, mc.vrPlayer.vrdata_room_pre.getController(0));
                        float f = vec21.x;
                        float f1 = vec21.y;
    ...
    public static void orientOverlay(boolean guiRelative)
    {
        ...
        if (Showing)
        {
            ...
            if (mc.vrSettings.physicalKeyboard)
            {
                Vec3 vec3 = mc.vrPlayer.vrdata_room_pre.hmd.getPosition();
                Vec3 vec31 = new Vec3(0.0D, -0.5D, 0.3D);
                vec31 = vec31.yRot((float)Math.toRadians((double)(-mc.vrPlayer.vrdata_room_pre.hmd.getYaw())));
                Pos_room = new Vec3(vec3.x + vec31.x, vec3.y + vec31.y, vec3.z + vec31.z);
                float f = (float)Math.PI + (float)Math.toRadians((double)(-mc.vrPlayer.vrdata_room_pre.hmd.getYaw()));
            ...
            else
            {
                ...
                Vec3 vec36 = mc.vrPlayer.vrdata_room_pre.hmd.getCustomVector(vec34);
                Pos_room = new Vec3(vec36.x / 2.0D + vec33.x, vec36.y / 2.0D + vec33.y, vec36.z / 2.0D + vec33.z);
                Vec3 vec32 = mc.vrPlayer.vrdata_room_pre.hmd.getPosition();
